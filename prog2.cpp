...
Task2Res solveTask2(int n1, int n2, int seed) {
    ...
    if (n2 > 0) {
        int last1 = res.shift_arr1.back();
        for (int i = n2 - 1; i > 0; --i) {
            res.shift_arr1[i] = res.shift_arr1[i - 1];
        }
        res.shift_arr1[0] = last1;

        int last2 = res.shift_arr2.back();
        for (int i = n2 - 1; i > 0; --i) {
            res.shift_arr2[i] = res.shift_arr2[i - 1];
        }
        res.shift_arr2[0] = last2;
    }
